...
// once saved (see PATCH /api/checklists/[id]), so this UI only ever needs
// to produce well-shaped data, never sanitize it itself.
export function BlockInspector({ block, onChange, recipes, images, onImageUploaded }: {
...
function ImageUploadForm({ onUploaded }: { onUploaded: (image: GuideImage) => void }) {
  ...
  async function upload() {
    ...
    try {
      ...
      const res = await fetch('/api/guide-images', { method: 'POST', body: form })
