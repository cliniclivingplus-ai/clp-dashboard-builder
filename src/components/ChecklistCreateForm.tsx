...
export default function ChecklistCreateForm({ patientId, sessionId, onCreated }: { patientId: string; sessionId?: string; onCreated: (checklistId: string) => void }) {
  ...
  useEffect(() => {
    fetch('/api/recipe-bank').then((r) => r.json()).then((j) => setRecipes(Array.isArray(j) ? j : [])).catch(() => {})
    fetch('/api/guide-images').then((r) => r.json()).then((j) => setImages(Array.isArray(j) ? j : [])).catch(() => {})
  }, [])
