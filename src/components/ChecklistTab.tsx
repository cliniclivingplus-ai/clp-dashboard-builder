...
export default function ChecklistTab({ patientId }: { patientId: string }) {
  ...
  useEffect(() => {
    fetch(`/api/checklists?patient_id=${patientId}`)
      .then((r) => r.json())
    ...
    <div>
      ...
      {showCreate && (
        <div style={{ marginBottom: 16 }}>
          <ChecklistCreateForm patientId={patientId} onCreated={(id) => router.push(`/patients/${patientId}/checklist/${id}`)} />
        </div>
      )}
      ...
      {checklists.length > 0 && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {checklists.map((c) => (
            <button key={c.id} onClick={() => router.push(`/patients/${patientId}/checklist/${c.id}`)}
